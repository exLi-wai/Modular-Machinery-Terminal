...
public class PacketComponentGuiPager implements IMessage {
    ...
    public PacketComponentGuiPager(MachineKey key, ComponentGuiGroup group, int index, int total) {
        this.key = key;
        this.group = group;
        this.index = index;
        this.total = total;
    ...
    public static class Handler implements IMessageHandler<PacketComponentGuiPager, IMessage> {
        ...
        public IMessage onMessage(PacketComponentGuiPager message, MessageContext ctx) {
            Minecraft.getMinecraft().addScheduledTask(() -> ComponentGuiPager.set(message.key, message.group, message.index, message.total));
            return null;
        }
    }
}
